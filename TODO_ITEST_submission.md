...
Стан на 2026-09-19: зроблено все, що робиться в репозиторії, і внесено правки розд. 0
у `submission/SZabolotnii_ITEST_paper_10p.docx` та `paper/draft_uk.md`
(перелік — у `submission/CHANGES_14p_to_10p.md`). Лишились Zenodo і перевірка у Word.
...
- [x] **Табл. 2**, колонка ARE(PMM3) — значення вище.
- [x] **Розд. 3.3, абзац після Табл. 2** — «PMM3 is robust, maintaining ARE ≈ 1.0 for all
...
- [x] **Розд. 3.3, останнє речення** про збіжність — «100% of the 500 replications …
...
- [x] **Внесок (iii) у вступі** — «PMM3 maintains ARE ≈ 1 under model misspecification».
- [x] **Conclusions, пункт «Robustness»** — те саме «ARE ≈ 1».
- [x] **Abstract** — «PMM3 is shown to be robust under misspecification» можна лишити,
      але варто додати число (ARE 1.02–2.68).

Інші розбіжності тексту з даними (від пайплайну не залежать):
- [x] **Межа бімодальності.** Розд. 2.2 («λ > 1/√2 — the distribution becomes bimodal»),
...
- [x] **Iris, розд. 3.2**: «g₃ theor = 0.517 which corresponds to a 51.7% reduction».
      Має бути g₃ theor = 0.483 (1 − 0.483 = 51.7%).
- [x] **Negative example, розд. 3.2**: «four further real datasets with γ₄ ∈ [−0.54, −0.12],
...
- [x] перевставлено в docx: Рис. 1 ← `Fig1_2_TN_densities_efficiency.png`,
      Рис. 2 ← `Fig3_g3_convergence.png`, Рис. 3 ← `Fig4_ARE_comparison.png`;
      підпис Рис. 1 оновлено (λ = 1). Fig3/Fig4 експортуються 18×9.8 і 18×10.8 см,
      щоб пропорції збігались із версткою статті
...
- [x] Табл. 2 (робастність) — нові значення внесено
...
- [ ] відкрити 10p.docx у Word, Ctrl+A → F9 (поля SEQ; кеш уже виставлено: Табл. 1–2, рівняння (1)–(8));
